...
var RootCmd = &cobra.Command{
	Use:   "ckeeper",
	Short: "Keep your containerized application healthy",
	Long:  `A smart tool`,
...
func initConfig() {
	logger.Info("root initConfig() called\n")
	pFlags := RootCmd.PersistentFlags()

	pFlags.StringVar(&cfgFile, "config", "",
		"config file (default name is ckeeper.yaml, will search paths of $HOME, /etc/, ./ or GOPATH/pkg)")
	pFlags.String("logging-level", "DEBUG", "logging level: DEBUG, INFO, WARNING, ERROR")
	pFlags.String("host-daemon", "unix:///var/run/docker.sock", "Daemon address to the container host")
	...
	if cfgFile != "" { // enable ability to specify config file via flag
		...
		for _, p := range filepath.SplitList(gopath) {
			projPath := filepath.Join(p, "src/github.com/yeasy/ckeeper")
	...
	if logLevel, err := logging.LogLevel(loggingLevel); err != nil {
		...
		logging.SetLevel(logLevel, "ckeeper")
	}
